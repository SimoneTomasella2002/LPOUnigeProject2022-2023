...
import Progetto_Gruppo_33.parser.ast.*;

import static Progetto_Gruppo_33.parser.TokenType.*;
...
public class MyLangParser implements Parser {
	...
	*/
	private ForEachStmt parseForEachStmt() throws ParserException {
		consume(FOREACH); // Consumes the 'foreach' keyword
		final var var = parseVariable(); // Parses the loop variable (IDENT)
		consume(IN); // Consumes the 'in' keyword
		return new ForEachStmt(var, parseExp(), parseBlock()); // Constructs and returns the ForEachStmt object
	}
	...
	private Exp parseAtom() throws ParserException {
		...
		case OPEN_VECT -> parseVector();	// Character used to load parseVector() method
 ...
 */
	...
	private VectorLiteral parseVector() throws ParserException{
		consume(OPEN_VECT); // Consumes the '[' character
		final Exp firstExp = parseExp(); // Parses the first expression inside the vector
		consume(SEPARATOR); // Consumes the ';' character
		final Exp secondExp = parseExp(); // Parses the second expression inside the vector
		consume(CLOSE_VECT); // Consumes the ']' character
		return new VectorLiteral(firstExp, secondExp); // Constructs and returns the VectorLiteral object
	}
