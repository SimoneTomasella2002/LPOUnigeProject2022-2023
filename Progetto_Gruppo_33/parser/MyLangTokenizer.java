...
public class MyLangTokenizer implements Tokenizer {
	...
	private static final Map<String, TokenType> symbols = new TreeMap<>(); // symbol table
	...
	static { // initialization of the symbol and keyword tables: symbols and keywords are singleton lexical categories 
		...
		symbols.put("[", OPEN_VECT);	// Added new symbol into the hashmap
		symbols.put("]", CLOSE_VECT);	// Added new symbol into the hashmap
		...
		keywords.put("foreach", FOREACH);	// Added new Keyword into the hashmap
		keywords.put("in", IN);				// Added new Keyword into the hashmap
	}
	...
	static { 
		...
		for (var s : symbols.keySet())
			symbolList.addFirst("\\" + String.join("\\", s.split(""))); // every char is pre-pended with "\\" to avoid regular expression syntax problems
		final var symbolRegEx = "(" + String.join("|", symbolList) + ")"; // group 1: symbols
		...
		final var keywordRegEx = "(" + String.join("|", keywords.keySet()) + "\\b)"; // group 2: keywords
